...
BEGIN 
  ...
  IF _id = 0 THEN
    INSERT INTO datosCotizacion (numeroRecotizacion, cotizacion_id, descripcion_doc, revicion_inicial, cantidad_doc, HHUnidades, total,observacion, HH_asociado, proveerdor, viatico, list_docs_id)
    VALUES (_numeroRecotizacion, _cotizacion_id, _descripcion_doc, revicion, _cantidad_doc,_HHUnidades,_total,_observacion,_HH_asociado,_proveerdor,_viatico,_list_docs_id);

    SELECT id INTO _idControl FROM control WHERE cotizacion_id = _cotizacion_id;

    INSERT INTO datosControl (descripcion_doc, revicion_inicial, cantidad_doc, HHUnidades, total,observacion, modificar_lista, proveedor, viatico, control_id, control_cotizacion_id, list_docs_id)
    VALUES (_descripcion_doc,revicion,_cantidad_doc,_HHUnidades,_total,_observacion,_modificar_lista,_proveerdor,_viatico,_idControl,_cotizacion_id,_list_docs_id);
    ...
    VALUES (_idCerti,_idControl,_cotizacion_id,0,0,0,0,0,0,_list_docs_id);
  ...
  ELSE
    ...
    revicion_inicial = revicion,
    cantidad_doc = _cantidad_doc,
    HHUnidades = _HHUnidades,
    total = _total,
    observacion = _observacion,
    modificar_lista = HH_asociado,
